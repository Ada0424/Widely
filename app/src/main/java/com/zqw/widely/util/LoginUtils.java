...
public class LoginUtils {
    ...
    public static void checkLogin(boolean needLogin){
        if(isLogin()){
            ...
            Users account = BmobUser.getCurrentUser(BaseApplication.getInstance(),Users.class);
            if(account == null){
...
//        BaseApplication.getInstance().startActivity(new Intent(BaseApplication.getInstance(), LoginActivity.class));
        BaseApplication.getInstance().startActivity(new Intent("com.zqw.widely.view.activity").setFlags(Intent.FLAG_ACTIVITY_NEW_TASK));
    }

    public static void autoLogin(){
        String userPho = PreferencesManager.getInstance(BaseApplication.getInstance()).get(Common.USER_PHOTO);
